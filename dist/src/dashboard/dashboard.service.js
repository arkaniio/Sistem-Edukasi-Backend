...
let DashboardService = class DashboardService {
    ...
    async getSummary(teacherId) {
        ...
        const activeCbtCount = await this.prisma.cBTTest.count({
    ...
    async getPendingTasks(teacherId) {
        return [];
    }
    async getStudentSummary(userId) {
        ...
        const totalAttendances = await this.prisma.attendance.count({ where: { studentId: student.id } });
        const presentCount = await this.prisma.attendance.count({ where: { studentId: student.id, status: 'PRESENT' } });
        const attendanceRate = totalAttendances === 0 ? 0 : Math.round((presentCount / totalAttendances) * 100);
        const classSubjects = await this.prisma.classSubject.findMany({ where: { classId: student.classId } });
